import os, asyncio, re, shutil, random, math, time
import hashlib
...
def update_if_new_version():
    local_hash = None
    if os.path.exists(LOCAL_FILE):
        with open(LOCAL_FILE, "rb") as f:
            local_hash = get_sha256(f.read())

    r = requests.get(RAW_URL)
    if r.status_code != 200:
        print(f"⚠️ Failed to fetch remote file: {r.status_code}")
        return False

    remote_content = r.content
    remote_hash = get_sha256(remote_content)
